...
//V, L vectors
float Viewer[3] = {-40,-30,-40};
float Light[3] = {-100,-100,-100};

//Incident light
float iLight[3] = {1.0 ,1.0, 1.0};

//Colour for each vertex used in Goraud Illumination
vector<vector<float> > goraudInt;
...
public:
	...
	//Change in RGB value with the change in Y
	float IYSlope[3];
...
class TransformToPerspective
{
	...
	void CreateUVN()
	{
		...
		div=vecOps.Normalize(Viewer);
		Viewer[0] = Viewer[0]/div;
		Viewer[1] = Viewer[1]/div;
		Viewer[2] = Viewer[2]/div;
...
public:
	...
	//Fills the vertex colours for each vertex
	void FillGoraudInt(Model model)
	{
		for(int i=0;i<model.numOfPoints;i++)
		{
			vector<float> a;
			...
			normal[0] = model.vertexNormalArray[i][0];
			normal[1] = model.vertexNormalArray[i][1];
			normal[2] = model.vertexNormalArray[i][2];
			...
			for(int k=0; k<3;k++)
			{
				amb = iLight[k] * model.kA[k];
				amb=0;

				diff = ( iLight[k] * model.kD[k] * nl );

				spec = ( iLight[k] * model.kS[k] * nh );
			...
			goraudInt.push_back(a);
		}

		if(goraudInt.size() < model.numOfPoints)
		{
			cout<<"\nERROR LOG : Goraud Intensities not fully filled";
		}
	}
...
class ScanConversion
{
	...
	void InsertionSort()
	{
		...
		for(i=1;i<AET.size();i++)
		{
			...
			if(ShadingModel == 2)
			{
				temp.IYSlope[0] = AET[i].IYSlope[0];
				temp.IYSlope[1] = AET[i].IYSlope[1];
				temp.IYSlope[2] = AET[i].IYSlope[2];

				temp.iOfYMin[0] = AET[i].iOfYMin[0];
				temp.iOfYMin[1] = AET[i].iOfYMin[1];
				temp.iOfYMin[2] = AET[i].iOfYMin[2];
			}
			...
			while((temp.xOfYMin < AET[j].xOfYMin) && (j >= 0))
			{
				...
				if(ShadingModel == 2)
				{
					AET[j+1].IYSlope[0] = AET[j].IYSlope[0];
					AET[j+1].IYSlope[1] = AET[j].IYSlope[1];
					AET[j+1].IYSlope[2] = AET[j].IYSlope[2];

					AET[j+1].iOfYMin[0] = AET[j].iOfYMin[0];
					AET[j+1].iOfYMin[1] = AET[j].iOfYMin[1];
					AET[j+1].iOfYMin[2] = AET[j].iOfYMin[2];
				}
			...
			if(ShadingModel == 2)
			{
				AET[j+1].IYSlope[0] = temp.IYSlope[0];
				AET[j+1].IYSlope[1] = temp.IYSlope[1];
				AET[j+1].IYSlope[2] = temp.IYSlope[2];

				AET[j+1].iOfYMin[0] = temp.iOfYMin[0];
				AET[j+1].iOfYMin[1] = temp.iOfYMin[1];
				AET[j+1].iOfYMin[2] = temp.iOfYMin[2];
			}
	...
	void UpdateXZ()
	{
		for(int i=0;i<AET.size();i++)
		{
			...
			if(ShadingModel == 2)
			{
				AET[i].iOfYMin[0] = AET[i].iOfYMin[0] + AET[i].IYSlope[0];
				AET[i].iOfYMin[1] = AET[i].iOfYMin[1] + AET[i].IYSlope[1];
				AET[i].iOfYMin[2] = AET[i].iOfYMin[2] + AET[i].IYSlope[2];
			}
			if(ShadingModel == 3)
			{
...
public:
	...
	vector<vector<vector<Edge> > > FillEdgeTable(Model model)
	{
		...
		//Fill the colour table for each vertex in goraud shading
		if(ShadingModel ==	2)
		{
			goraudInt.clear();
			ill.FillGoraudInt(model);
		}
		...
		for(int i=0;i<model.numOfPoly;i++)
		{
			...
			for(int j=1;j<numOfSides+1;j++)
			{
				...
				if(start != 0)
				{
					...
					//Initial values for Goraud and Phong shading
					if(ShadingModel == 2)
					{
						i1[0] = goraudInt[start-1][0];	i1[1] = goraudInt[start-1][1];	i1[2] = goraudInt[start-1][2];
						i2[0] = goraudInt[end-1][0];	i2[1] = goraudInt[end-1][1];	i2[2] = goraudInt[end-1][2];
					}

					if(ShadingModel == 3)
					{
						n1[0] = model.vertexNormalArray[start-1][0];	n1[1] = model.vertexNormalArray[start-1][1];	n1[2] = model.vertexNormalArray[start-1][2];
						n2[0] = model.vertexNormalArray[end-1][0];	n2[1] = model.vertexNormalArray[end-1][1];	n2[2] = model.vertexNormalArray[end-1][2];
					}
					...
					if(y1>y2)
					{
						...
						if(ShadingModel == 2)
						{
							e.iOfYMin[0] = i2[0];	e.iOfYMin[1] = i2[1];	e.iOfYMin[2] = i2[2];
						}
						if(ShadingModel == 3)
						{
							e.normOfYMin[0] = n2[0];	e.normOfYMin[1] = n2[1];	e.normOfYMin[2] = n2[2];
						}
					}
					else
					{
						...
						if(ShadingModel == 2)
						{
							e.iOfYMin[0] = i1[0];	e.iOfYMin[1] = i1[1];	e.iOfYMin[2] = i1[2];
						}
						if(ShadingModel == 3)
						{
							e.normOfYMin[0] = n1[0];	e.normOfYMin[1] = n1[1];	e.normOfYMin[2] = n1[2];
						}
					}

					//Colour and normal slope along each edge
					if(ShadingModel == 2)
					{
						e.IYSlope[0]= (float)((float)(i1[0]-i2[0])/(float)(y1-y2));
						e.IYSlope[1]= (float)((float)(i1[1]-i2[1])/(float)(y1-y2));
						e.IYSlope[2]= (float)((float)(i1[2]-i2[2])/(float)(y1-y2));
					}
	...
	void ScanlineFill(vector<vector<vector<Edge> > > mT, Model model)
	{
		...
		for(int k=0;k<mT.size();k++)
		{
			...
			if(ShadingModel == 1)
			{
				RGB = ill.ConstantShading(model,k);
			...
			for(i=0;i<windowHeight;i++)
			{
				...
				while(j < AET.size())
				{
					...
					if(coordCount%2 == 0)
					{
						...
						if(ShadingModel == 2)
						{
							i1[0] = (AET[j].iOfYMin[0]);	i1[1] = (AET[j].iOfYMin[1]);	i1[2] = (AET[j].iOfYMin[2]);
						}
						if(ShadingModel == 3)
						{
							n1[0] = (AET[j].normOfYMin[0]);	n1[1] = (AET[j].normOfYMin[1]);	n1[2] = (AET[j].normOfYMin[2]);
						}
					...
					else
					{
						...
						if(ShadingModel == 2)
						{
							i2[0] = (AET[j].iOfYMin[0]);	i2[1] = (AET[j].iOfYMin[1]);	i2[2] = (AET[j].iOfYMin[2]);
						}

						if(ShadingModel == 3)
						{
							n2[0] = (AET[j].normOfYMin[0]);	n2[1] = (AET[j].normOfYMin[1]);	n2[2] = (AET[j].normOfYMin[2]);
						}
					...
					if(fillFlag)
					{
						...
						else
						{
							...
							if(ShadingModel == 2)
							{
								IXSlope[0] = ((float)(i1[0]-i2[0])/(float)(x1-x2));
								IXSlope[1] = ((float)(i1[1]-i2[1])/(float)(x1-x2));
								IXSlope[2] = ((float)(i1[2]-i2[2])/(float)(x1-x2));
							}
							if(ShadingModel == 3)
							{
						...
						for(int a=x1;a<=x2;a++)
						{
							...
							//For Goraud shading the colour value is interpolated for the pixel.
							if(ShadingModel == 2)
							{
								ip[0]=ip[0] + IXSlope[0];	ip[1]=ip[1] + IXSlope[1];	ip[2]=ip[2] + IXSlope[2];
							...
							if(ShadingModel == 3)
							{
								...
								np[0]=np[0] + NormXSlope[0];	np[1]=np[1] + NormXSlope[1];	np[2]=np[2] + NormXSlope[2];

								phongColour = ill.GetPhongColour(model,np);

								r=phongColour[0];	g=phongColour[1];	b=phongColour[2];
							}
