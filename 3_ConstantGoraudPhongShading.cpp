...
public:
	...
	float kA[3] = {0.3,0.1,0.0};
	float kD[3] = {0.8,0.5,0.0};
...
class TransformToPerspective
{
	...
	void CreateUVN()
	{
		...
		for(int j = 0; j<3;j++)
		{
			H[j] = Light[j] + Viewer[j];
		}

		temp2=vecOps.Normalize(H);
		for(int j = 0; j<3;j++)
		{
			H[j] = H[j] / temp2;
		}
		...
		for(int i=0;i<model.numOfPoly;i++)
		{
			...
			for(int j=1;j<numOfSides+1;j++)
			{
				...
				if(start != 0)
				{
					...
					if(ShadingModel == 2)
					{
						i1[0] = goraudInt[start-1][0];
						i1[1] = goraudInt[start-1][1];
						i1[2] = goraudInt[start-1][2];

						i2[0] = goraudInt[end-1][0];
						i2[1] = goraudInt[end-1][1];
						i2[2] = goraudInt[end-1][2];
					}
			...
			for(i=0;i<windowHeight;i++)
			{
				...
				while(j < AET.size())
				{
					...
					if(coordCount%2 == 0)
					{
						...
						if(ShadingModel == 2)
						{
							i1[0] = (AET[j].iOfYMin[0]);
							i1[1] = (AET[j].iOfYMin[1]);
							i1[2] = (AET[j].iOfYMin[2]);
						}
						if(ShadingModel == 3)
						{
					...
					else
					{
						...
						if(ShadingModel == 2)
						{
							i2[0] = (AET[j].iOfYMin[0]);
							i2[1] = (AET[j].iOfYMin[1]);
							i2[2] = (AET[j].iOfYMin[2]);
						}
						...
						for(int a=x1;a<=x2;a++)
						{
							...
							if(zp<zBuffer[a][i])
							{
								...
								if(ShadingModel == 3)
								{
									...
									np[0]=cArr[0]/divN;	np[1]=cArr[1]/divN;	np[2]=cArr[2]/divN;
